...
for i in $@;do
  if [[ $count -ne 1 && $count -ne $# ]]; then
    printf "${i}_GeracaoJacobiana, ${i}_MetodoNewton, ${i}_CalculaJacobiana, ${i}_SistemaLinear" >> $OUT/L3.csv
    printf "${i}_GeracaoJacobiana, ${i}_MetodoNewton, ${i}_CalculaJacobiana, ${i}_SistemaLinear" >> $OUT/L2CACHE.csv
    printf "${i}_GeracaoJacobiana_dp, ${i}_GeracaoJacobiana_avx, ${i}_MetodoNewton_dp, ${i}_MetodoNewton_avx, ${i}_CalculaJacobiana_dp, ${i}_CalculaJacobiana_avx, ${i}_SistemaLinear_dp, ${i}_SistemaLinear_avx" >> $OUT/FLOPS_DP.csv
  fi
  count=$(( $count + 1))
done
printf "\n" >> $OUT/L3.csv
